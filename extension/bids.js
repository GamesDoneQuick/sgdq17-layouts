...
const BIDS_URL = nodecg.bundleConfig.useMockData ?
	...
	'https://gamesdonequick.com/tracker/search/?type=allbids&event=19';
const CURRENT_BIDS_URL = nodecg.bundleConfig.useMockData ?
	...
	'https://gamesdonequick.com/tracker/search/?type=allbids&feed=current&event=19';
	...
	if (!error && response.statusCode === 200) {
		...
		for (const id in parentBidsById) {
			...
			bid.options = bid.options.sort((a, b) => {
				const aTotal = numeral(a.total);
				const bTotal = numeral(b.total);
				if (aTotal > bTotal) {
					return -1;
				}
				if (aTotal < bTotal) {
